...
func TestDefine(t *testing.T) {
	...
	firstLocalScope := NewEnclosedSymbolTable(global)
	c := firstLocalScope.Define("c")
	if c != expected["c"] {
		t.Errorf("expected c=%+v, got=%+v", expected["c"], c)
	}

	d := firstLocalScope.Define("d")
	if d != expected["d"] {
		t.Errorf("expected d=%+v, got=%+v", expected["d"], d)
	}

	secondLocalScope := NewEnclosedSymbolTable(firstLocalScope)
	e := secondLocalScope.Define("e")
	if e != expected["e"] {
		t.Errorf("expected e=%+v, got=%+v", expected["e"], e)
	}

	f := secondLocalScope.Define("f")
	if f != expected["f"] {
...
func TestResolve(t *testing.T) {
	...
	for _, sym := range expected {
		...
		if !ok {
			t.Errorf("name %s cannot be resolved", sym.Name)
		}
