...
func TestCompilationScope(t *testing.T) {
	...
	if compiler.symbolTable != globalSymbolTable {
		t.Errorf("compiler did not resolve global symbol table")
	}
	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []any{
				...
				[]code.Instructions{
					code.Make(code.OpGetLocal, 0), // A needs to be loaded on the stack,altough it is not used directly, as the enclosed function is refrencing it.
